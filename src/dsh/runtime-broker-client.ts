...
export function runtimeBrokerPaths(globalStoragePath: string): RuntimeBrokerPaths {
  const id = createHash('sha256').update(globalStoragePath).digest('hex').slice(0, 20)
  return {
...
export async function tryAcquireRuntimeBroker(options: {
  ...
  do {
    const lease = await connectAndAcquire(options.paths.socket, options.port, undefined, 500, 65_000).catch((error: unknown) => {
      if (error instanceof BrokerRejectedError && error.code !== 'launcher-required') throw error
      return null
    })
  ...
  try {
    while (Date.now() < deadline) {
      ...
      if (!ownsLaunchLock) {
        ...
        if (ownsLaunchLock) {
          if (process.platform !== 'win32') await unlink(options.paths.socket).catch(() => undefined)
          const broker = spawn(process.execPath, [options.brokerScript, options.paths.socket, options.paths.metadata], {
            detached: true,
            stdio: 'ignore',
            windowsHide: true,
    ...
    if (ownsLaunchLock) await unlink(options.paths.launchLock).catch(() => undefined)
  }
}
...
async function tryTakeLaunchLock(path: string): Promise<boolean> {
  try {
    ...
    if (info && Date.now() - info.mtimeMs > 75_000) await unlink(path).catch(() => undefined)
    return false
  }
}
