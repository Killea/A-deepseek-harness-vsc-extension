...
export class WireClient {
  ...
  async call<T>(method: string, payload: unknown, signal?: AbortSignal): Promise<T> {
    ...
    if (message.type !== 'server-response' || message.rpcId !== rpcId) {
      throw new Error(`dsh web RPC ${method} 响应信封不匹配 (type=${message.type as string})`)
    }
  ...
  async respond(rpcId: string, result: RpcResult<unknown>): Promise<RpcReceipt> {
    ...
    if (!response.ok) throw new Error(`dsh web respond 载体错误: HTTP ${response.status}`)
    const parsed: unknown = await response.json()
    const receipt = parsed as RpcReceipt
    if (receipt === null || typeof receipt !== 'object' || typeof receipt.accepted !== 'boolean') {
...
export class EventStream extends EventEmitter {
  ...
  constructor(options: EventStreamOptions, wsImpl: new (url: string) => WsWebSocket) {
    super()
  ...
  private connect(): void {
    ...
    socket.onmessage = (event) => {
      ...
      if (message.type !== 'server-request') {
        this.emit('error', new Error(`事件帧 type 异常: ${String(message.type)}`))
        return
      }
