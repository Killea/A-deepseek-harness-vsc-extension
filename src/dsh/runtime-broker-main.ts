...
async function startIpc(): Promise<void> {
  ...
  ipc.listen(socketPath, () => {
    if (process.platform !== 'win32') void chmod(socketPath, 0o600).catch(() => undefined)
  })
}

function requiredArgument(value: string | undefined): string {
  if (value === undefined || value === '') throw new Error('Runtime Broker 缺少启动参数')
  return value
}
...
async function handleAcquire(socket: Socket, raw: string): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      || request.port < 1
      || request.port > 65_535
    ) throw new RuntimeBrokerError('configuration', '无效的 Broker acquire 请求')
...
async function ensureRuntime(port: number, launcher?: DshLauncher): Promise<RuntimeState> {
...
async function bootRuntime(port: number, launcher?: DshLauncher): Promise<RuntimeState> {
  const baseUrl = loopbackDshUrl(port)
  const existing = await probeDsh(baseUrl)
  if (existing.kind === 'dsh') {
    const state = { baseUrl, port, server: null, reportedVersion: existing.description.version }
  ...
  if (!launcher) throw new RuntimeBrokerError('launcher-required', '启动 DSH 需要可执行文件信息')
  ...
  try {
    ...
    if (winner.kind === 'dsh') {
      const state = { baseUrl, port, server: null, reportedVersion: winner.description.version }
      await publishMetadata(state)
      return state
    }
    if (await isTcpPortOccupied('127.0.0.1', port)) {
  ...
  const state = { baseUrl, port, server, reportedVersion: verified.description.version }
  await publishMetadata(state)
...
function scheduleShutdownIfIdle(): void {
  ...
  shutdownTimer = setTimeout(() => { void shutdown() }, 1_500)
}
