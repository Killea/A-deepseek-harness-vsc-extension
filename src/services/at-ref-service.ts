...
export class AtRefService {
  ...
  async listCandidates(): Promise<AtRefCandidate[]> {
    ...
    for (const file of files) {
      ...
      candidates.push({ absolutePath: file, relativePath: rel, pinned: false, dirty: false })
    }
   ...
   */
  async buildReference(candidate: AtRefCandidate, sessionId: string | null): Promise<AtRefResult> {
    const workspaceRoot = this.deps.workspaceRoot()
  ...
  async verifyBaseline(sessionId: string | null): Promise<BaselineInfo> {
    ...
    const sessionCwd = await this.deps.sessionCwd(sessionId)
    const match =
      workspaceRoot !== '' && sessionCwd !== null ? samePath(sessionCwd, workspaceRoot) : null
    if (match === false) {
      this.deps.onLog?.(
        `会话工作目录（${sessionCwd}）与当前工作区（${workspaceRoot}）不一致：` +
          `历史会话的 @ 引用将使用绝对路径`,
  ...
  listProblemCandidates(): AtProblemCandidate[] {
    ...
    const pinned = this.deps.activeEditor()
    const rows: Array<AtProblemCandidate & { sortKey: string; line: number; column: number; pinned: boolean }> = []
    ...
    return rows.map(({ path, line, column, severity, message }) => ({ path, line, column, severity, message }))
  }
  ...
  private async buildMatcher(workspaceRoot: string, files: string[]): Promise<GitignoreMatcher> {
    const rules: GitignoreRule[] = []
    for (const file of files) {
      if (basename(file) !== '.gitignore') continue
      const relDir = toPosix(relative(workspaceRoot, file))
      const baseDir = relDir === '.gitignore' ? '' : relDir.slice(0, -'.gitignore'.length - 1)
      let content = ''
      try {
