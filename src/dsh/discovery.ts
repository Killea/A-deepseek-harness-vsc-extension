...
async function fromNpmPrefix(): Promise<DshLauncher | null> {
  const prefix = await new Promise<string | null>((resolve) => {
    execFile('npm', ['prefix', '-g'], { timeout: 10_000, windowsHide: true }, (error, stdout) => {
  ...
  for (const name of candidateNames()) {
    ...
    if (isExecutable(candidate)) return { command: candidate, args: [], source: 'npm-prefix' }
  }
  return null
}
...
export async function discoverDsh(options: {
  ...
  const launcher = fromConfigHit ?? fromPath() ?? (await fromNpmPrefix()) ?? fromNpx()
