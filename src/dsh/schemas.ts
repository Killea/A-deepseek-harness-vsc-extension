...
export class RuntimeSchemaValidator implements EnvelopeValidator {
  ...
  validateServerResponse(value: unknown): boolean {
    ...
    return schema === undefined || (schema as { safeParse(v: unknown): { success: boolean } }).safeParse(value).success
  }
 ...
 */
async function loadSchemaModules(installRoot: string): Promise<SchemaModules | null> {
  const tryRequire = (): SchemaModules | null => {
    try {
      ...
      return {
        rpcSchema: { serverResponseSchema: rpcSchema.serverResponseSchema, serverRequestSchema: rpcSchema.serverRequestSchema },
        eventsSchema: { muxFrameSchema: eventsSchema.muxFrameSchema, hostFrameSchema: eventsSchema.hostFrameSchema },
      }
    } catch {
      return null
    }
  ...
  try {
    const require = createRequire(join(installRoot, 'package.json'))
    const rpcPath = require.resolve('@deepseek-ai/dsh-host-apiproxy/api/rpc.schema')
    const eventsPath = require.resolve('@deepseek-ai/dsh-host-apiproxy/api/events.schema')
 ...
 */
export async function createWireValidator(launcher: DshLauncher): Promise<EnvelopeValidator | null> {
