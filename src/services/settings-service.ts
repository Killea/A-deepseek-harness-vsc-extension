...
function schemaNodeAt(
  ...
  for (const key of path) {
    ...
    if (node.type === 'object') node = node.dict?.[key] === undefined ? undefined : refs[node.dict[key]]
...
export class SettingsService {
  ...
  async loadPanel(): Promise<SettingsPanelView> {
    ...
    try {
      const [providersResult, describeResult] = await Promise.all([
        client.call<{ providers: ConfigurableProviderView[] }>('llm.providers', {}),
    ...
    const rows: SettingsProviderRowView[] = providers.map((entry) => {
      ...
      if (namespace !== undefined) {
        const pinned = getPath(namespace.base, [...entry.settingsPath, 'models'])
        inheritedModels = pinned ?? schemaDefaultAt(namespace.schema, [...entry.settingsPath, 'models'])
      }
      return {
    ...
    if (refs.length > 0) {
      try {
        const result = await client.call<{ credentials: Record<string, CredentialView> }>(
    ...
    return {
      ...
      namespaces: Object.fromEntries(
        describe.namespaces.map(view => [view.ns, {
          ns: view.ns,
          value: view.value,
  ...
  async applyProfile(profile: SettingsProfileApply): Promise<SettingsWriteResult> {
    ...
    if (profile.keyValue.length > 0) {
      try {
        await client.call('credentials.set', { ref: profile.keyRef, value: profile.keyValue })
      } catch (error) {
  ...
  async removeProvider(target: SettingsRemoveTarget): Promise<SettingsWriteResult> {
    const client = this.requireClient()
  ...
  async declareProvider(create: SettingsProviderCreate): Promise<SettingsWriteResult> {
    const client = this.requireClient()
    const keyRef = deriveKeyRef(create.route)
    const storesKey = create.keyValue.length > 0
    const profile: Record<string, unknown> = {
      ...create.displayName.length === 0 ? {} : { displayName: create.displayName },
    ...
    if (storesKey) {
      try {
        await client.call('credentials.set', { ref: keyRef, value: create.keyValue })
      } catch (error) {
        return { ok: false, text: `凭据保存失败: ${messageOf(error)}` }
      }
    }
