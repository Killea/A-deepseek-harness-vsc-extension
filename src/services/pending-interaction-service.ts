...
import type { PendingAnswer, PendingItemView, PendingQuestionView } from '../shared/protocol.ts'
...
export class PendingInteractionService extends EventEmitter {
   ...
   */
  async answer(sessionId: string, key: string, answer: PendingAnswer): Promise<RpcReceipt> {
  ...
  async cancel(sessionId: string, key: string): Promise<RpcReceipt> {
    ...
    if (entry.view.kind === 'approval') {
      throw new Error('审批请求没有取消出口（wire 仅 allowed-once/rejected 两结局）')
    }
    const client = this.requireClient()
    return await client.respond(entry.rpcId, {
      ok: false,
  ...
  private upsert(key: string, sessionId: string, rpcId: string, view: PendingItemView): void {
  ...
  private requireEntry(sessionId: string, key: string): PendingEntry {
    const entry = this.entries.get(key)
    if (!entry || entry.sessionId !== sessionId) throw new Error('待应答交互不存在或已结算')
    return entry
  }
 ...
 */
function narrowQuestions(key: string, questions: WireQuestion[]): PendingItemView {
...
function planReviewOf(questions: WireQuestion[]): Omit<Extract<PendingItemView, { kind: 'plan-review' }>, 'kind' | 'key'> | null {
