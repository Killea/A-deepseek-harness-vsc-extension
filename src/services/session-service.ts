...
export class SessionService {
  ...
  async ensureWorkspace(folderRoot: string): Promise<WorkspaceView> {
    ...
    const created = await client.call<{ workspace: WorkspaceView; created: boolean }>('workspace.create', {
      path: folderRoot,
    })
   ...
   */
  async listSessions(selectedSessionId?: string | null): Promise<SessionSummary[]> {
    ...
    const { items: workspaces, archivedSessionIds } = await client.call<{ items: WorkspaceView[]; archivedSessionIds: string[] }>('workspace.list', {})
    ...
    const { items } = await client.call<{ items: SessionSummary[] }>('session.list', {})
    return items
   ...
   */
  async resolveNewSession(occupiedBlankSessionIds: readonly string[] = []): Promise<{ sessionId: string }> {
    const workspace = this.workspace
    ...
    return await client.call<{ sessionId: string }>('session.create', { workspaceId: fresh.workspaceId })
  }
  ...
  async archiveSession(sessionId: string): Promise<readonly string[]> {
    const client = this.requireClient()
    const { archivedSessionIds } = await client.call<{ archivedSessionIds: string[] }>('workspace.archiveSession', { sessionId })
    this.archivedSessionIds = archivedSessionIds
  ...
  async renameSession(sessionId: string, title: string): Promise<string> {
    const client = this.requireClient()
    const result = await client.call<{ title: string; seq: number }>('session.rename', { sessionId, title })
    return result.title
  }
