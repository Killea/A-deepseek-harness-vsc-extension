...
import type { CommandDescriptorView, SessionModelsView } from '../shared/protocol.ts'
...
export class CommandService {
  ...
  private readonly inFlight = new Map<string, Promise<CommandDescriptorView[]>>()
  ...
  async commandsAvailable(sessionId: string): Promise<boolean> {
    ...
    try {
      await client.call<unknown>('commands/list', { args: { agentId: sessionId } })
      this.available = true
    } catch (error) {
  ...
  async execute(sessionId: string, line: string): Promise<CommandExecutionResult | null> {
    const client = this.requireClient()
    const value = await client.call<RawCommandExecution | undefined>('commands/execute', {
      args: { agentId: sessionId, line },
    })
    if (!value) return null
    return { commandId: value.commandId, kind: value.result.kind, text: value.result.text }
  }

  /** session.models 投影（/model 弹出层 + 输入框下方席位共享的目录快照）。 */
  async models(sessionId: string): Promise<SessionModelsView> {
    const client = this.requireClient()
    const raw = await client.call<RawSessionModels>('session.models', { sessionId })
    return {
      current:
        raw.current === null
          ? null
          : {
              ...
              ...(raw.current.reasoningEffort === undefined ? {} : { reasoningEffort: raw.current.reasoningEffort }),
            },
      ...
      failures: (raw.failures ?? []).map((f) => ({ id: f.id, name: f.name, message: f.message })),
      error: null,
    }
  }

  /** session.selectModel（/model 选中 / 席位切换；effort 缺席 = 回落模型默认）。 */
  async selectModel(sessionId: string, provider: string, model: string, effort?: string): Promise<void> {
    const client = this.requireClient()
