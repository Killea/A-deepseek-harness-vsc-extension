...
afterEach(async () => {
  for (const wss of websocketServers.splice(0)) await new Promise<void>(resolve => wss.close(() => resolve()))
  for (const server of servers.splice(0)) await new Promise<void>(resolve => server.close(() => resolve()))
})

describe('probeDsh', () => {
  it('recognizes the DSH RPC envelope and both event downlinks without rejecting its version', async () => {
    const baseUrl = await startFixture(['/api/events.mux', '/api/events.host'], '0.0.1')
  ...
  it('does not recognize an endpoint when one DSH downlink is missing', async () => {
    ...
    await expect(probeDsh(baseUrl)).resolves.toMatchObject({ kind: 'not-dsh', baseUrl })
  })

  it('rejects credentials and non-root paths in external URLs', () => {
    expect(() => normalizeDshBaseUrl('https://user:secret@example.com:3080')).toThrow(/用户名或密码/u)
    expect(() => normalizeDshBaseUrl('https://example.com:3080/dsh')).toThrow(/根路径/u)
  })
})

async function startFixture(upgradePaths: string[], version: string): Promise<string> {
