...
export type ExtensionToWebviewMessage =
  ...
  | { type: 'atCandidates'; sessionId: string | null; requestId: number; candidates: AtCandidatesView }
  // M3b: @ 选中后由扩展侧构造的插入文本（文件走基准计算，问题走内联 @problem）。
  | { type: 'atResult'; sessionId: string | null; requestId: number; insert: string }
  // M3b: / 命令目录快照（契约跟随：available=false 时 `/` 当普通文本，菜单不呼出）。
  | { type: 'commands'; sessionId: string | null; requestId: number; snapshot: CommandsSnapshot }
  // M3b+: / 菜单技能目录快照（skill.list；available=false 或失败 → 技能为空，不关闭菜单）。
  | { type: 'skills'; sessionId: string | null; requestId: number; snapshot: SkillsSnapshot }
  // M3b: /model 弹出层数据（session.models 投影）。
  | { type: 'models'; sessionId: string | null; requestId: number; models: SessionModelsView }
  // M3b: 命令准入的即时反馈（未知命令 / 传输失败等，不进入对话流）。
  | { type: 'commandNotice'; sessionId: string | null; level: 'info' | 'error'; text: string }
  ...
  | { type: 'sessionActivity'; sessionId: string; activity: SessionActivityView }
  ...
  | { type: 'settingsReply'; id: number; ok: false; text: string; conflict?: boolean }
...
export type WebviewToExtensionMessage =
  ...
  | { type: 'atResolve'; sessionId: string | null; requestId: number; ref: AtRefPayload }
  // M3b: / 菜单打开 → 扩展侧（重新）拉取命令目录（目标会话由扩展侧裁决）。
  | { type: 'commandOpen'; sessionId: string | null; requestId: number; occupiedBlankSessionIds?: string[] }
  ...
  | { type: 'pendingAnswer'; sessionId: string; key: string; answer: PendingAnswer }
