...
import { ConversationFold, type WireSessionEvent } from '../conversation/fold.ts'
import type { WireClient, ServerRequest } from '../dsh/wire.ts'
import type { ConversationItem, ConversationSnapshot } from '../shared/protocol.ts'
...
export class ConversationService extends EventEmitter {
  ...
  private readonly onProjections?: (sessionId: string, block: ProjectionsBlock) => void
  ...
  applyFrame(frame: ServerRequest): void {
    ...
    if (fold.applyIfNewer(payload.event)) this.emit('change', payload.sessionId)
  }

  /** Re-attach every tracked session (mux reconnect; see module doc). */
  async resync(): Promise<void> {
    for (const sessionId of [...this.folds.keys()]) await this.attach(sessionId)
  }
  ...
  private withAgentErrorNotes(sessionId: string, snapshot: ConversationSnapshot): ConversationSnapshot {
    const notes = this.agentErrors.get(sessionId)
    if (!notes || notes.length === 0) return snapshot
    const items: ConversationItem[] = [...snapshot.items]
    for (const message of notes) items.push({ kind: 'note', text: `会话出错：${message}` })
