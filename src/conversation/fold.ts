...
function foldBlocks(blocks: Map<number, BlockAccum>): { text: string; reasoning: string } {
  let text = ''
  let reasoning = ''
...
function extractContent(content: ContentBlock[]): { text: string; reasoning: string } {
  let text = ''
  let reasoning = ''
...
function collect(record: Record<string, unknown>, member: string, field: string): string[] {
...
function contextForm(source: Record<string, unknown> | null): ContextFormName | null {
  const form = source === null ? null : readString(source, 'form')
  return form !== null && (CONTEXT_FORMS as readonly string[]).includes(form) ? form as ContextFormName : null
}

/** instructions form：对账文件变更列表；all-or-nothing（一条不可读 → 整体 opaque）。 */
function instructionChanges(source: Record<string, unknown>): InstructionChangeView[] | null {
  ...
  for (const entry of list) {
    ...
    if (path === null || (action !== 'set' && action !== 'replace' && action !== 'remove')) return null
...
function catalogEntries(source: Record<string, unknown>): CatalogEntryView[] | null {
...
function snapshotSections(source: Record<string, unknown>): SnapshotSectionView[] | null {
...
function toolResultText(message: ToolResultData['message']): string {
  ...
  for (const item of nested) {
    ...
    if (entry !== null && entry['type'] === 'text' && typeof entry['text'] === 'string') text += entry['text']
  }
  return text
}
...
function windowBytes(lines: { text?: unknown }[]): number {
  ...
  for (const line of lines) {
    if (typeof line.text === 'string') total += Buffer.byteLength(line.text, 'utf8') + 1
  }
  return total
}
...
function envelopeLines(text: string): { number: number; text: string }[] | null {
...
function readResultView(meta: unknown, text: string): ToolResultView | null {
  ...
  return {
    ...
    lines,
    totalLines: typeof totalLines === 'number' && totalLines >= 0 ? totalLines : -1,
    ...(typeof lang === 'string' && lang !== '' ? { lang } : {}),
  }
}

/** M5: 逐行 diff（LCS 回溯）——按块输出变更行（删除块 → 新增块）。 */
function lineDiff(oldText: string | null, newText: string | null): { changes: WriteChangeView[]; additions: number; deletions: number } {
  const a = oldText === null || oldText === '' ? [] : oldText.replace(/\n$/u, '').split('\n')
  const b = newText === null || newText === '' ? [] : newText.replace(/\n$/u, '').split('\n')
  const n = a.length
  const m = b.length
  ...
  if (n * m > 4_000_000) {
    ...
    for (let i = prefix; i < n - suffix; i++) changes.push({ kind: 'del', text: a[i] ?? '' })
    for (let i = prefix; i < m - suffix; i++) changes.push({ kind: 'add', text: b[i] ?? '' })
    return { changes, additions: m - prefix - suffix, deletions: n - prefix - suffix }
  }
...
function writeDiffMeta(value: unknown): { oldText: string | null; newText: string | null } | null {
  const record = asRecord(value)
...
function writeResultView(meta: unknown, args: Record<string, unknown> | null): ToolResultView | null {
  const diff = writeDiffMeta(meta)
  if (diff !== null) {
    const { changes, additions, deletions } = lineDiff(diff.oldText, diff.newText)
...
export class ConversationFold {
  ...
  apply(event: WireSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case 'assistant/chunk': {
        ...
        if (chunk.type === 'usage' || chunk.type === 'finish' || chunk.type === 'tool-call-delta') break
      ...
      case 'assistant/message': {
        ...
        this.items.push({ kind: 'assistant', text, ...(reasoning === '' ? {} : { reasoning }), partial: false })
        this.revision++
        break
      }
      case 'turn/start':
      ...
      case 'turn/end': {
        ...
        if (reason?.kind === 'aborted') {
          ...
          this.items.push({ kind: 'note', text: `出错：${reason.error?.message ?? reason.message ?? '未知错误'}` })
          changed = true
        }
      ...
      case 'command/done': {
        ...
        const index = this.items.findIndex(
          (item) => item.kind === 'command' && item.commandId === data.commandId,
        )
        const current = index >= 0 ? this.items[index] : undefined
        if (current && current.kind === 'command') {
          this.items[index] = {
            ...current,
            outcome: { kind: data.kind, ...(data.text !== undefined ? { text: data.text } : {}) },
          }
        } else {
      ...
      case 'tool/result': {
        ...
        const failed = (block !== null && block['isError'] === true) || data.error !== undefined
        ...
        const index = callId === null ? -1 : this.items.findIndex(
        ...
        if (current !== undefined && current.kind === 'tool' && !failed) {
          if (current.name === 'read') {
            ...
            if (meta !== null) {
              ...
              if (path !== undefined) {
                const bytes = meta.lines === undefined ? undefined : windowBytes(meta.lines)
                summary = {
                  path,
                  ...(meta.totalLines !== undefined ? { lines: meta.totalLines } : {}),
  ...
  snapshot(): ConversationSnapshot {
    return { items: [...this.items], running: this.running, lastSeq: this.lastSeq }
  }
  ...
  private closePartial(turn?: number, step?: number): void {
    ...
    this.open = null
    if (turn !== undefined && step !== undefined && open.turn === turn && open.step === step) {
  ...
  private applyChunk(chunk: StreamChunk): void {
    ...
    switch (chunk.type) {
      case 'block-start': {
        ...
        blocks.set(chunk.index, { kind: chunk.blockType === 'reasoning' ? 'reasoning' : 'text', text: '' })
        break
      }
      case 'text-delta': {
        const prev = blocks.get(chunk.index)
        blocks.set(chunk.index, { kind: 'text', text: (prev?.kind === 'text' ? prev.text : '') + chunk.text })
        break
      }
      case 'reasoning-delta': {
        const prev = blocks.get(chunk.index)
        blocks.set(chunk.index, { kind: 'reasoning', text: (prev?.kind === 'reasoning' ? prev.text : '') + chunk.text })
        break
      }
      case 'block-end': {
