...
export function normalizeDshBaseUrl(raw: string): string {
  ...
  if (url.username || url.password) throw new Error('DSH 地址不能包含用户名或密码')
 ...
 */
export async function probeDsh(baseUrlInput: string, timeoutMs = 3_000): Promise<DshProbeResult> {
  ...
  try {
    ...
    const descriptionPromise = client.call<unknown>('host.describe', {}, abort.signal)
    ...
    if (!description) {
      return { kind: 'not-dsh', baseUrl, reason: 'host.describe 响应结构不符合 DSH 契约' }
    }
...
export async function isTcpPortOccupied(host: string, port: number, timeoutMs = 800): Promise<boolean> {
  assertPort(port)
...
export function parseDshHostDescription(value: unknown): DshHostDescription | null {
