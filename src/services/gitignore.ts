...
/** 解析单行；空行/纯注释返回 null。 */
export function parseGitignoreLine(rawLine: string): Omit<GitignoreRule, 'baseDir'> | null {
  let line = rawLine
...
export class GitignoreMatcher {
  ...
  constructor(rules: GitignoreRule[]) {
    ...
    this.rules = [...rules]
      ...
      .map((rule) => ({ ...rule, regex: compilePattern(rule.pattern, rule.anchored) }))
  }
...
function matchRule(rule: GitignoreRule, relPath: string, isDir: boolean): boolean {
