...
import { EventStream, WireClient, type EnvelopeValidator, type ServerRequest } from '../dsh/wire.ts'

export type DshStatus = 'discovering' | 'starting' | 'ready' | 'reconnecting' | 'stopped' | 'error'
export type DshOwnership = 'external-specified' | 'external-discovered' | 'external-managed-port' | 'managed'
...
export class DshService extends EventEmitter {
  ...
  get statusValue(): DshStatus { return this.status }
  get baseUrl(): string | null { return this.currentBaseUrl }
  get client(): WireClient | null { return this.wire }
  get launcherValue(): DshLauncher | null { return this.launcher }
  get ownershipValue(): DshOwnership | null { return this.ownership }
  get reportedVersionValue(): string | null { return this.reportedVersion }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.validator = target.ownership === 'managed' && this.launcher
  ...
  private async resolveTarget(): Promise<ResolvedTarget> {
    ...
    if (external) {
      ...
      if (result.kind !== 'dsh') throw new Error(`指定地址不是可用的 DSH：${result.reason}`)
      return {
    ...
    this.options.onLog?.(`默认端口未发现 DSH (${discoveryUrl}): ${discovered.reason}`)
    ...
    if (managed.kind === 'dsh') {
      ...
      this.brokerLease = await tryAcquireRuntimeBroker({ paths, port: this.options.managedPort })
      if (this.brokerLease) {
        return {
    ...
    if (await isTcpPortOccupied('127.0.0.1', this.options.managedPort)) {
      throw new Error(`DSH 管理端口 ${String(this.options.managedPort)} 已被其他程序占用：${managed.reason}`)
    }
  ...
  private async openGeneration(): Promise<void> {
    ...
    const mux = new EventStream({ url: `${wsBase}/api/events.mux`, validator: this.validator ?? undefined }, WebSocket)
    const host = new EventStream({ url: `${wsBase}/api/events.host`, validator: this.validator ?? undefined }, WebSocket)
    this.wire = wire
    this.mux = mux
    this.host = host
    ...
    mux.on('error', (error: Error) => this.options.onLog?.(`[events.mux] ${error.message}`))
    host.on('error', (error: Error) => this.options.onLog?.(`[events.host] ${error.message}`))
    let ready = false
    ...
    try {
      ...
      if (closedWhileOpening) throw new Error('DSH WebSocket 在连接代际就绪前关闭')
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimer = setTimeout(() => {
      ...
      void this.openGeneration().then(
        () => this.setStatus('ready'),
        (error: unknown) => {
          this.options.onLog?.(`dsh 重连失败: ${error instanceof Error ? error.message : String(error)}`)
          this.scheduleReconnect()
        },
      )
    }, 1_000)
...
function waitForEventOpen(stream: EventStream, timeoutMs = 10_000): Promise<void> {
