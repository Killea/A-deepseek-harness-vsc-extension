 ...
 *   1. explicit path from configuration (`killea.deepseek-gold-harness.dshPath`)
 ...
 * @param options.explicitPath - `killea.deepseek-gold-harness.dshPath` override, if set.
 */
    ...
    if (launcher.source === "npx") {
      throw new Error(
        "未找到 dsh：PATH、npm 全局目录均无 dsh，且 `npx --no-install @deepseek-ai/dsh` 不可用。" +
          " 请先安装：`npm install -g @deepseek-ai/dsh`，或在设置 killea.deepseek-gold-harness.dshPath 中指定路径。",
      );
    }
    throw new Error(
