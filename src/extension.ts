...
import { AtRefService, type ProblemDiagnostic } from './services/at-ref-service.ts'
import { CommandService } from './services/command-service.ts'
import { SkillService } from './services/skill-service.ts'
import { SettingsService, deriveSettingsYamlPath } from './services/settings-service.ts'
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const settings = new SettingsService({ wire: () => dsh.client, onLog: (line) => log(line) })
  ...
  const pickDshPath = async (): Promise<void> => {
    ...
    try {
      ...
      void vscode.window.showErrorMessage(`dsh 重启失败: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  const atRef = new AtRefService({
    ...
    activeEditor: () => {
      ...
      return { path: editor.document.uri.fsPath, dirty: editor.document.isDirty }
    },
  ...
  dsh.on('mux', (frame) => {
    ...
    const payload = frame.payload as { type?: string; error?: { message?: string } }
  ...
  dsh.on('host', (frame) => {
    ...
    if (frame.method === 'host/session-added' || frame.method === 'host/workspace-changed') {
    ...
    if (frame.method === 'host/agent-error') {
      ...
      if (payload.sessionId && payload.message) provider.pushAgentError(payload.sessionId, payload.message)
    }
    // M4: host 流级错误 → 日志 + 状态栏。
    ...
    if (frame.method === 'host/remote-event') {
      const payload = frame.payload as { type?: string; event?: string; args?: unknown[] }
      if (payload.type === 'host/remote-event' && payload.event === 'commands/change') {
        commands.invalidate()
        if (provider.selectedSessionId) void provider.refreshCommands(provider.selectedSessionId)
      }
      ...
      if (payload.type === 'host/remote-event' && payload.event === 'agent-preset/selected') {
        ...
        if (typeof sessionId === 'string') {
          ...
          if (provider.selectedSessionId === sessionId) void provider.refreshSkills(sessionId)
        }
      }
    }
  })
  dsh.on('muxClose', () => {
    ...
    conversations.resync().catch((error: unknown) => log(`会话重同步失败: ${String(error)}`))
  })

  const provider = new ChatViewProvider(
    sessions, conversations, projections, atRef, commands, skills, pending, settings, dshFacts, pickDshPath, restartDsh,
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('weinibuliu.dsh-vsc.openInBrowser', async () => {
    ...
    vscode.commands.registerCommand('weinibuliu.dsh-vsc.newSession', async () => {
      await provider.handleMessage({ type: 'newSession' })
    }),
    vscode.commands.registerCommand('weinibuliu.dsh-vsc.refreshSessions', async () => {
  ...
  if (autoStart) {
    dsh.start().catch((error: unknown) => {
      ...
      void vscode.window.showErrorMessage(`dsh 启动失败: ${error instanceof Error ? error.message : String(error)}`)
    })
  }
